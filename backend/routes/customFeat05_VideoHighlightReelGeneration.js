...
// Auto-compile best shots from gallery into video highlight reels (with music, transitions); saves hours of manual editing
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhotographyBusinessManager / cf-video-highlight-reel-generation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_video_highlight_reel_generation`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-video-highlight-reel-generation feature
router.post('/run', authenticateToken, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Video highlight reel generation'.
Context: Auto-compile best shots from gallery into video highlight reels (with music, transitions); saves hours of manual editing
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-video-highlight-reel-generation/run error:', err);
