...
// Gap Feature: No photographer schedule optimization
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhotographyBusinessManager / gap-no-photographer-schedule-optimization',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No photographer schedule optimization'.\nProject: AIPhotographyBusinessManager.\nContext: No photographer schedule optimization\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-photographer-schedule-optimization', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-photographer-schedule-optimization', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
