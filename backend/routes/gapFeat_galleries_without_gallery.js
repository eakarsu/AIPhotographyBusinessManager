...
// Gap Feature: Galleries without \'/gallery
// Galleries without /gallery-organization-ai (auto-sort and tag images)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhotographyBusinessManager / gap-galleries-without-gallery',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Galleries without \'/gallery'.\nProject: AIPhotographyBusinessManager.\nContext: Galleries without /gallery-organization-ai (auto-sort and tag images)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['galleries-without-gallery', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'galleries-without-gallery', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
