...
// Gap Feature: No client proofing workflow (advanced markup/approval)
// No client proofing workflow (advanced markup/approval)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhotographyBusinessManager / gap-no-client-proofing-workflow-advanced-markup-approv',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No client proofing workflow (advanced markup/approval)'.\nProject: AIPhotographyBusinessManager.\nContext: No client proofing workflow (advanced markup/approval)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-client-proofing-workflow-advanced-markup-approv', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-client-proofing-workflow-advanced-markup-approv', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-client-proofing-workflow-advanced-markup-approv']
    );
