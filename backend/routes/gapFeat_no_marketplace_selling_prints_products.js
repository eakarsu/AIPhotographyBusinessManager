...
// Gap Feature: No marketplace (selling prints, products)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhotographyBusinessManager / gap-no-marketplace-selling-prints-products',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No marketplace (selling prints, products)'.\nProject: AIPhotographyBusinessManager.\nContext: No marketplace (selling prints, products)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-marketplace-selling-prints-products', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-marketplace-selling-prints-products', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
