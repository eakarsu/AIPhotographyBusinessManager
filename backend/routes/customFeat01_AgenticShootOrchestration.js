...
// Pre-shoot, AI suggests props, locations, lighting setups based on style; during shoot, AI monitors in real-time (via preview), suggests angle/composition corrections; post-shoot, auto-culls, color-gra
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhotographyBusinessManager / cf-agentic-shoot-orchestration',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_shoot_orchestration`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-agentic-shoot-orchestration feature
router.post('/run', authenticateToken, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic shoot orchestration'.
Context: Pre-shoot, AI suggests props, locations, lighting setups based on style; during shoot, AI monitors in real-time (via preview), suggests angle/composition corrections; post-shoot, auto-culls, color-grades, captions
Project: AIPhotographyBusinessManager.
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-shoot-orchestration/run error:', err);
