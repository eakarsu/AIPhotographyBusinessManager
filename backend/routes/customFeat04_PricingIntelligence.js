...
// Track competitor pricing by service type/location; AI suggests optimal pricing strategy; predict client willingness-to-pay based on budget signals
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhotographyBusinessManager / cf-pricing-intelligence',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_pricing_intelligence`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-pricing-intelligence feature
router.post('/run', authenticateToken, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Pricing intelligence'.
Context: Track competitor pricing by service type/location; AI suggests optimal pricing strategy; predict client willingness-to-pay based on budget signals
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-pricing-intelligence/run error:', err);
