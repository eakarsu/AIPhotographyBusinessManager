...
// AI generates personalized emails (booking confirmations, post-shoot thank yous, gallery delivery); monitors sentiment (tone detection) to auto-adjust follow-up strategy
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhotographyBusinessManager / cf-client-communication-automation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_client_communication_automation`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-client-communication-automation feature
router.post('/run', authenticateToken, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Client communication automation'.
Context: AI generates personalized emails (booking confirmations, post-shoot thank yous, gallery delivery); monitors sentiment (tone detection) to auto-adjust follow-up strategy
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-client-communication-automation/run error:', err);
