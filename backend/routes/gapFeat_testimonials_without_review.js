...
// Gap Feature: Testimonials without \'/review
// Testimonials without /review-sentiment-analyze (monitor client sentiment)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhotographyBusinessManager / gap-testimonials-without-review',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Testimonials without \'/review'.\nProject: AIPhotographyBusinessManager.\nContext: Testimonials without /review-sentiment-analyze (monitor client sentiment)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['testimonials-without-review', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'testimonials-without-review', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
