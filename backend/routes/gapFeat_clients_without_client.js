...
// Gap Feature: Clients without \'/client
// Clients without /client-satisfaction-predict (flag at-risk clients)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhotographyBusinessManager / gap-clients-without-client',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Clients without \'/client'.\nProject: AIPhotographyBusinessManager.\nContext: Clients without /client-satisfaction-predict (flag at-risk clients)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['clients-without-client', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'clients-without-client', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
