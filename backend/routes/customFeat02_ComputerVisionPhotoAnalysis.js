...
// Batch process shoot; AI rates each image on technical (sharpness, exposure, composition) and emotional (client impact) metrics; auto-selects gallery-ready shots
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhotographyBusinessManager / cf-computer-vision-photo-analysis',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_computer_vision_photo_analysis`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-computer-vision-photo-analysis feature
router.post('/run', authenticateToken, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Computer vision photo analysis'.
Context: Batch process shoot; AI rates each image on technical (sharpness, exposure, composition) and emotional (client impact) metrics; auto-selects gallery-ready shots
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-computer-vision-photo-analysis/run error:', err);
