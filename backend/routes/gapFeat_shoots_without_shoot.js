...
// Gap Feature: Shoots without \'/shoot
// Shoots without /shoot-plan-optimize (suggest timeline, crew, locations)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhotographyBusinessManager / gap-shoots-without-shoot',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Shoots without \'/shoot'.\nProject: AIPhotographyBusinessManager.\nContext: Shoots without /shoot-plan-optimize (suggest timeline, crew, locations)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['shoots-without-shoot', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'shoots-without-shoot', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
