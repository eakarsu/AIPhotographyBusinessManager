...
// Gap (Non-AI): No photographer schedule optimization
// No photographer schedule optimization
import React, { useState } from 'react';

export default function GapNoPhotographerScheduleOptimizationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-photographer-schedule-optimization/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No photographer schedule optimization</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No photographer schedule optimization</p>
