...
// Gap (Non-AI): No integration with Lightroom/Capture One (editing tools)
// No integration with Lightroom/Capture One (editing tools)
import React, { useState } from 'react';

export default function GapNoIntegrationWithLightroomCaptureOneEditingPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-integration-with-lightroom-capture-one-editing-/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No integration with Lightroom/Capture One (editing tools)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No integration with Lightroom/Capture One (editing tools)</p>
