...
// Gap (AI): Clients without '/client
// Clients without /client-satisfaction-predict (flag at-risk clients)
import React, { useState } from 'react';

export default function GapClientsWithoutClientPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-clients-without-client/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Clients without '/client</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Clients without /client-satisfaction-predict (flag at-risk clients)</p>
