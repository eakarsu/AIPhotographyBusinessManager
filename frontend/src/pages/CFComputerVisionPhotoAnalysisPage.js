...
// Custom Feature: Computer vision photo analysis
// Computer vision photo analysis: Batch process shoot; AI rates each image on technical (sharpness, exposure, composition) and emotional (client impact) metrics; auto-selects gallery-ready shots
import React, { useState } from 'react';

export default function CFComputerVisionPhotoAnalysisPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-computer-vision-photo-analysis/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Computer vision photo analysis</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Computer vision photo analysis: Batch process shoot; AI rates each image on technical (sharpness, exposure, composition) and emotional (client impact) metrics; auto-selects gallery-ready shots</p>
