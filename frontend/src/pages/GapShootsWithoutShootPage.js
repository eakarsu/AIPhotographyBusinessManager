...
// Gap (AI): Shoots without '/shoot
// Shoots without /shoot-plan-optimize (suggest timeline, crew, locations)
import React, { useState } from 'react';

export default function GapShootsWithoutShootPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-shoots-without-shoot/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Shoots without '/shoot</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Shoots without /shoot-plan-optimize (suggest timeline, crew, locations)</p>
