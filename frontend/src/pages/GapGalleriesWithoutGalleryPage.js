...
// Gap (AI): Galleries without '/gallery
// Galleries without /gallery-organization-ai (auto-sort and tag images)
import React, { useState } from 'react';

export default function GapGalleriesWithoutGalleryPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-galleries-without-gallery/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Galleries without '/gallery</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Galleries without /gallery-organization-ai (auto-sort and tag images)</p>
